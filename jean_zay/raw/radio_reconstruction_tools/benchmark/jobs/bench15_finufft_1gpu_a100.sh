#!/bin/bash
#SBATCH --job-name=rrt_bench15_finufft_1gpu_a100
...
# Timing benchmark: 15-layer finufft training on 1 A100 GPU(s), a few real
...
srun python -u scripts/benchmark/benchmark_training.py \
    ...
    --epochs 3 \
    --max-duration-min 90 \
    --label bench15_finufft_1gpu \
    --output-json $RESULTS_DIR/bench15_finufft_1gpu.json
