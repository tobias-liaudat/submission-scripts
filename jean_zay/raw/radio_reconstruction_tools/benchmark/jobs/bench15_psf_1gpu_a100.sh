#!/bin/bash
#SBATCH --job-name=rrt_bench15_psf_1gpu_a100
#SBATCH --mail-user=tobiasliaudat@gmail.com
#SBATCH --mail-type=ALL
#SBATCH --ntasks=1                   # 1 tache MPI par GPU (DDP: doit = --devices)
...
# Timing benchmark: 15-layer psf training on 1 A100 GPU(s), a few real
...
CONFIG=$SUBMISSION_REPO/jean_zay/raw/radio_reconstruction_tools/benchmark/configs/bench15_unrolled_psf_finufft.yaml
...
srun python -u scripts/benchmark/benchmark_training.py \
    ...
    --epochs 3 \
    --max-duration-min 90 \
    --label bench15_psf_1gpu \
    --output-json $RESULTS_DIR/bench15_psf_1gpu.json
