#!/bin/bash
#SBATCH --job-name=rrt_bench15_psf_4gpu_a100
...
# Timing benchmark: 15-layer psf training on 4 A100 GPU(s), a few real
...
srun python -u scripts/benchmark/benchmark_training.py \
    ...
    --epochs 3 \
    --max-duration-min 90 \
    --label bench15_psf_4gpu \
    --output-json $RESULTS_DIR/bench15_psf_4gpu.json
