#!/bin/bash
#SBATCH --job-name=rrt_bench15_tkbn_1gpu_a100
...
# Timing benchmark: 15-layer tkbn training on 1 A100 GPU(s), a few real
...
CONFIG=$SUBMISSION_REPO/jean_zay/raw/radio_reconstruction_tools/benchmark/configs/bench15_unrolled_tkbn.yaml
...
srun python -u scripts/benchmark/benchmark_training.py \
    ...
    --epochs 3 \
    --max-duration-min 90 \
    --label bench15_tkbn_1gpu \
    --output-json $RESULTS_DIR/bench15_tkbn_1gpu.json
