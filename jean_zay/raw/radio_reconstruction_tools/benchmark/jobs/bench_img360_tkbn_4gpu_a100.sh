#!/bin/bash
#SBATCH --job-name=rrt_bench_img360_tkbn_4gpu_a100
...
# Self-contained img360 timing benchmark (tkbn, 4-GPU DDP), sized for the dev
...
CONFIG=$SUBMISSION_REPO/jean_zay/raw/radio_reconstruction_tools/benchmark/configs/bench_img360_op_tkbn.yaml
...
srun --ntasks=4 --ntasks-per-node=4 python -u scripts/benchmark/benchmark_training.py \
    ...
    --epochs 2 \
    --max-duration-min 60 \
    --label bench_img360_tkbn_4gpu \
    --output-json $RESULTS_DIR/bench_img360_tkbn_4gpu.json
