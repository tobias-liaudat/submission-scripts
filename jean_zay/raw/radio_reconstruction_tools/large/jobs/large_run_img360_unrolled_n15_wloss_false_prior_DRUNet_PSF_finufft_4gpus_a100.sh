#!/bin/bash
#SBATCH --job-name=rrt_large_img360_psf_wl0_4gpu
...
# ===========================================================================
# Self-chaining 4-GPU (DDP) A100 training job for the img360 large run (psf, weighted_loss=false).
...
#
# weighted_loss=false ablation of the weighted PSF run: same 15-layer net, same
# greedy 2 -> 15 schedule, same pinned condition — only the per-layer loss
# weights differ. Never mix its checkpoints with the weighted ones: everything
# (config, RUN_ID, wandb run) carries the _wloss_false token.
#
# NO BUILDER PREREQUISITE: this chain REUSES the artifacts dir already built for
# the weighted campaign (large_img360_cov1000_ntrain4000_psf_finufft, finufft /
# briggs stamp, incl. the 4 GB PSF bank). It is read-only there, so it may run
# concurrently with the weighted chain. Submit it directly with sbatch.
# Launch:  cd .../large/jobs/logs && sbatch ../large_run_img360_unrolled_n15_wloss_false_prior_DRUNet_PSF_finufft_4gpus_a100.sh
# ===========================================================================
...
JOB_SCRIPT=$SUBMISSION_REPO/jean_zay/raw/radio_reconstruction_tools/large/jobs/large_run_img360_unrolled_n15_wloss_false_prior_DRUNet_PSF_finufft_4gpus_a100.sh
CONFIG=$SUBMISSION_REPO/jean_zay/raw/radio_reconstruction_tools/large/configs/large_run_img360_unrolled_n15_wloss_false_prior_DRUNet_PSF_finufft_4gpus.yaml
OUTPUT_ROOT=/lustre/fswork/projects/rech/ney/ulx23va/projects/radio/outputs/trained_models

BASE_NAME=large_run_img360_unrolled_n15_wloss_false_prior_DRUNet_PSF_finufft_4gpus
