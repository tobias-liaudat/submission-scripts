#!/bin/bash
#SBATCH --job-name=rrt_large_img64_psf_wl0_4gpu
...
# ===========================================================================
# Self-chaining 4-GPU (DDP) A100 training job for the img64 large run (psf, weighted_loss=false).
...
# the weighted campaign (large_img64_cov1000_ntrain4000_psf_finufft, finufft /
# briggs stamp). It is read-only there, so it may run concurrently with the
# weighted chain. Submit it directly with sbatch — no dependency needed.
# Launch:  cd .../large/jobs/logs && sbatch ../large_run_img64_unrolled_n15_wloss_false_prior_DRUNet_PSF_finufft_4gpus_a100.sh
# ===========================================================================
...
JOB_SCRIPT=$SUBMISSION_REPO/jean_zay/raw/radio_reconstruction_tools/large/jobs/large_run_img64_unrolled_n15_wloss_false_prior_DRUNet_PSF_finufft_4gpus_a100.sh
CONFIG=$SUBMISSION_REPO/jean_zay/raw/radio_reconstruction_tools/large/configs/large_run_img64_unrolled_n15_wloss_false_prior_DRUNet_PSF_finufft_4gpus.yaml
OUTPUT_ROOT=/lustre/fswork/projects/rech/ney/ulx23va/projects/radio/outputs/trained_models

BASE_NAME=large_run_img64_unrolled_n15_wloss_false_prior_DRUNet_PSF_finufft_4gpus
