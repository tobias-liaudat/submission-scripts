#!/bin/bash
#SBATCH --job-name=rrt_large_img64_opfft_wl0_4gpu
#SBATCH --mail-user=tobiasliaudat@gmail.com
#SBATCH --mail-type=ALL
#SBATCH --ntasks=4                   # 1 tache MPI par GPU (DDP: doit = runtime.devices)
#SBATCH --ntasks-per-node=4
#SBATCH --gres=gpu:4                 # 4 GPU (demi-noeud A100: 8 GPU/noeud)
#SBATCH --cpus-per-task=8            # coeurs CPU par tache (un huitieme de noeud A100: 64/8)
...
#SBATCH -A rbn@a100                  # allocation A100 (radio reconstruction)

# ===========================================================================
# Self-chaining 4-GPU (DDP) A100 training job for the img64 large run (op finufft, weighted_loss=false).
...
#
# weighted_loss=false, explicit-NUFFT-gradient variant (model.type: unrolled,
# operator: finufft) — not the PSF one and not the existing tkbn one. The op
# data-fidelity gradient costs two NUFFTs per layer per step, so this chain is
# the slowest of the three and the most likely to hit the MAX_RESUBMITS=15 cap
# before finishing — if it stops there, bump the cap and resubmit with the same
# RUN_ID to continue the chain from its last checkpoint.
# Never mix its checkpoints with the weighted runs: everything (config, RUN_ID,
# wandb run) carries the _wloss_false token.
#
# NO BUILDER PREREQUISITE: this chain REUSES the artifacts dir already built for
# the weighted campaign — deliberately the *psf_finufft* one, whose derived
# stamp (finufft / briggs) matches this backend so the cached op-norms are
# loaded rather than recomputed; psf_bank.pt is simply never read by
# type: unrolled. Read-only there, so it may run concurrently with the PSF
# chains. Submit it directly with sbatch — no dependency needed.
# Launch:  cd .../large/jobs/logs && sbatch ../large_run_img64_unrolled_n15_wloss_false_prior_DRUNet_op_finufft_4gpus_a100.sh
# ===========================================================================
...
JOB_SCRIPT=$SUBMISSION_REPO/jean_zay/raw/radio_reconstruction_tools/large/jobs/large_run_img64_unrolled_n15_wloss_false_prior_DRUNet_op_finufft_4gpus_a100.sh
CONFIG=$SUBMISSION_REPO/jean_zay/raw/radio_reconstruction_tools/large/configs/large_run_img64_unrolled_n15_wloss_false_prior_DRUNet_op_finufft_4gpus.yaml
OUTPUT_ROOT=/lustre/fswork/projects/rech/ney/ulx23va/projects/radio/outputs/trained_models

BASE_NAME=large_run_img64_unrolled_n15_wloss_false_prior_DRUNet_op_finufft_4gpus
