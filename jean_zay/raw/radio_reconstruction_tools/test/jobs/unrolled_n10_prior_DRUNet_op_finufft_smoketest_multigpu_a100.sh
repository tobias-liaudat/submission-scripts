#!/bin/bash
#SBATCH --job-name=rrt_smoke_unrolled_DRUNet_finufft_multigpu_a100    # nom du job
#SBATCH --mail-user=tobiasliaudat@gmail.com
#SBATCH --mail-type=ALL
#SBATCH --ntasks=2                   # nombre total de tache MPI (= nombre total de GPU)
#SBATCH --ntasks-per-node=2          # 1 tache par GPU (DDP: doit = runtime.devices)
#SBATCH --gres=gpu:2                 # 2 GPU d'un noeud A100
...
# Multi-GPU (2x A100, single node, DDP) smoke test. srun launches 2 processes
# (one per GPU); Lightning's SLURM environment wires up the ranks from
# SLURM_PROCID. Launch AFTER a smoke job has populated the shared artifacts dir
# (submit_smoketests.sh handles the dependency); if the artifacts are missing,
# rank 0 builds them while the other rank waits.
...
CONFIG=$SUBMISSION_REPO/jean_zay/raw/radio_reconstruction_tools/configs/unrolled_n10_prior_DRUNet_op_finufft_smoketest_multigpu.yaml
...
#   $OUTPUT_ROOT/<run_name>_smoketest_multigpu_a100_<YYYYmmdd_HHMMSS>/stage_NNlayers/
...
# $OUTPUT_ROOT/${RUN_NAME}_smoketest_multigpu_a100_<ts>/; the shared offline
# wandb run under $OUTPUT_ROOT/wandb/.
srun python -u scripts/train_unrolled_cluster.py \
    --config $CONFIG \
    --device cuda \
    --log-dir $OUTPUT_ROOT \
    --run-name ${RUN_NAME}_smoketest_multigpu_a100
