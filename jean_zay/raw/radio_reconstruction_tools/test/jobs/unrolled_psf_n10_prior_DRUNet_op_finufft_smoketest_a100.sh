#!/bin/bash
#SBATCH --job-name=rrt_smoke_unrolled_psf_DRUNet_finufft_a100    # nom du job
...
#   $OUTPUT_ROOT/<run_name>_smoketest_a100_<YYYYmmdd_HHMMSS>/stage_NNlayers/
...
# $OUTPUT_ROOT/${RUN_NAME}_smoketest_a100_<ts>/; the shared offline wandb run
# under $OUTPUT_ROOT/wandb/.
srun python -u scripts/train_unrolled_cluster.py \
    --config $CONFIG \
    --device cuda \
    --log-dir $OUTPUT_ROOT \
    --run-name ${RUN_NAME}_smoketest_a100
