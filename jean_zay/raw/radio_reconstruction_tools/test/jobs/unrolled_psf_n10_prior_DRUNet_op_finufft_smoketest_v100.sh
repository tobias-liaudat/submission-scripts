#!/bin/bash
#SBATCH --job-name=rrt_smoke_unrolled_psf_DRUNet_finufft_v100    # nom du job
...
#SBATCH --qos=qos_gpu-dev            # dev queue (<= 2h, scheduling rapide) pour le test
...
CONFIG=$SUBMISSION_REPO/jean_zay/raw/radio_reconstruction_tools/configs/unrolled_psf_n10_prior_DRUNet_op_finufft_smoketest.yaml
...
#   $OUTPUT_ROOT/<run_name>_smoketest_<YYYYmmdd_HHMMSS>/stage_NNlayers/
...
RUN_NAME=unrolled_psf_n10_prior_DRUNet_op_finufft
...
# $OUTPUT_ROOT/${RUN_NAME}_smoketest_<ts>/; the shared offline wandb run under
# $OUTPUT_ROOT/wandb/.
srun python -u scripts/train_unrolled_cluster.py \
    --config $CONFIG \
    --device cuda \
    --log-dir $OUTPUT_ROOT \
    --run-name ${RUN_NAME}_smoketest_v100
