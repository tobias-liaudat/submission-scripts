#!/bin/bash
#SBATCH --job-name=rrt_validate_large_img64_artremoval
...
# Validate the finished img64 artifact_removal (UNet) large run: plots the training metrics, computes
...
BASE=large_run_img64_artifact_removal_unet_finufft_4gpus
RUN_DIR=""
...
# --n-eval / --batch-size tuned for this image size (img64 artifact_removal (UNet)).
