#!/bin/bash
#SBATCH --job-name=rrt_validate_large_img64_tkbn
...
# Validate the finished img64 tkbn (unrolled) large run: plots the training metrics, computes
...
BASE=large_run_img64_unrolled_n15_prior_DRUNet_op_tkbn_4gpus
RUN_DIR=""
...
# --n-eval / --batch-size tuned for this image size + backend (img64 tkbn (unrolled)).
